...
class ToolCenterPointInitializer(composer.Initializer):
  ...
  * Collisions between either the arm or hand and an external body without a
    ...
    for _ in range(self._max_rejection_samples):
      ...
      if success:
        ...
        if (self._ignore_collisions
          ...
          # return # This was the actual original code, but to return the target pos, i changed it a bit and added the above return target_pos line
  ...
  def check_target_feasibility(self, physics, random_state, target_pos, target_quat):
    """Checks whether a target position is in collision or is even reachable"""
    target_site = self._arm.wrist_site
    success = self._arm.set_site_to_xpos(
          physics=physics, random_state=random_state, site=target_site,
          target_pos=target_pos, target_quat=target_quat,
  ...
  def tcp_initializer_at_given_pose(self, physics, position, quaternion, random_state):
    """Sets initial tool center point pose via inverse kinematics, but not at a random state, 
      it sets it at the given coordinate.
    Args:
      physics: An `mjcf.Physics` instance.
      position: A variations.evaluate() result specifying the target position
      quaternion: A variations.evaluate() result specifying the target quaternion
      random_state: An `np.random.RandomState` instance. Not much useful here, but the set_site_to_xpos function
                    requires it so just using it. Ultimately even inside that function, it is merely a placeholder.
